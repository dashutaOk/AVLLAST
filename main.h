...
#ifndef AVLLAST_MAIN_H
#define AVLLAST_MAIN_H
...
namespace TreeLib {
    ...
    public:
        ...
        template<bool IsConst = false>
                ...
                public:
                    ...
                    typedef const std::shared_ptr<Node<KeyT, ValT>> pointer;
                    ...
                    Iterator<IsConst>& operator--() {
                        ...
                        if (node == nullptr) {
                            node_ = AVLMap::GetRight(tree_root_.lock());
                        }
                        if (node->left != nullptr) {
                            node_ = NextLeft(node->left);
                        } else {
                            node_ = NextTopMinus(node->parent);
                        }
                        return *this;
                    }
        ...
        std::string ToString(const std::string& order, bool ignore_values = false) {
            std::ostringstream ss;
            OrderGo(root_, order, [&ss, &ignore_values] (const KeyT& key, const ValT& val) {
                if (ignore_values) {
                    ss << key << " ";
                } else {
                    ss << key << " " << val << " ";
        ...
        static AVLMap<KeyT, ValT> FromString(const std::string& s) {
            AVLMap<KeyT, ValT> result;
            std::istringstream iss(s);
            ValT val;
            KeyT key;
            while ((iss >> key) && (iss >> val)) {
    ...
    private:
        ...
        static void OrderGo(
                std::weak_ptr<Node<KeyT, ValT>> root,
                const std::string& order,
                const std::function<void(const KeyT&, ValT&)>& callback
                ) {
            if (!root.expired()) {
                auto ptr = root.lock();
                for (auto& ch : order) {
                    switch (ch) {
                        case 'L': // left
                        OrderGo(ptr->left, order, callback);
                        break;
                        case 'R': // right
                        OrderGo(ptr->right, order, callback);
                        break;
                        case 'C': // root
    ...
    public:
        ...
        ~Set() = default;
        Set(Set&&) = default;
        Set<T>& operator = (Set<T>&& other)  noexcept = default;

        void Insert(const T& x) {
            data_.Insert(x, nullptr);
        }
        ...
        std::string ToString() {
            return data_.ToString("LCR", true);
        }
